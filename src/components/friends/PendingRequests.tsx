  ...
  useEffect(() => {
    ...
    const init = () => {
      client.friendRequests.fetchAll().then(async (requests) => {
        ...
        let promises = await Promise.allSettled(incoming.map(r => {
        ...
        const extendedIncoming = incoming.map((r, i) => {
          ...
          return { request: r, user: (promises[i] as PromiseFulfilledResult<User>).value }
        });
        ...
        {incoming.map((request) => {
          return (
            <div key={request.request.id}>
              <p>{request.user?.username || "Unknown user"}</p>
