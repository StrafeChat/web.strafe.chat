...
// Stable audio player component using @solid-primitives/keyed
const StableAudioPlayer: Component<{ src: string; name: string; size: number; messageId?: string }> = (props) => {
  // Create a stable key based on the audio file properties
  const stableKey = createMemo(() => `${props.messageId || 'unknown'}-${props.src}-${props.name}-${props.size}`);
...
export const Message: Component<MessageProps> = (props) => {
  ...
  const getInviteCodes = () => {
    ...
    return matches
      ? matches.map((m) => m.split("/").pop()).filter(Boolean)
      : [];
  };

  // Load invite infos using cache
  createEffect(async () => {
  ...
  const renderSystemMessage = () => {
    if (!props.system_type || !props.system_data)
      return {
        text: "System message",
        timestamp: formatTimeOnly(props.created_at),
      };
  ...
  const canDelete = createMemo(() => {
    ...
    if (currentRoom.type === RoomType.TEXT_ROOM) {
      return isAuthor;
    }
  ...
  const referencedMessages = createMemo(() => {
    ...
    return (
      props.message_references.map((refId) => {
                  ...
                  <span>
                    <Show when={messageData.actorId && messageData.actorName}>
                      <span
                        ...
                        onClick={(e) =>
                          handleSystemUserClick(e, messageData.actorId!)
                        }
                      >
                    ...
                    <Show when={messageData.targetId && messageData.targetName}>
                      <span
                        class="font-bold text-text-primary cursor-pointer hover:underline hover:text-white transition-colors"
                        onClick={(e) =>
                          handleSystemUserClick(e, messageData.targetId!)
                        }
                      >
          ...
          >
            <Show when={shouldShowCompact}>
              ...
              >
                <Tooltip
                  position="top"
                  content={formatFullDate(props.created_at)}
                >
