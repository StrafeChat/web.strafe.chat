...
import { TrackPublication, Participant } from "livekit-client";
import { VoiceHeader } from "./voice/VoiceHeader"; // Adjust path as per your project structure
import { Button } from "@/components/ui/button";

import { useClient, useVoice, useModal } from "@/hooks/";
...
export default function VoiceRoom(props: {
  ...
  const { connection, setConnection, setRoomInfo, disconnect, muteVoice } =
    useVoice();
...
function StrafeVoiceCall() {
  ...
  useEffect(() => {
    ...
    const userLeave = (user: Participant) => {
      ...
      const idxTrack = tracks.findIndex(
        (t) => t.user.identity === user.identity
      );
      if (idxTrack === -1) return setTracks([...tracks]);
    ...
    const addTrack = ({
      ...
      if (!users.find((u) => u.identity === participant.identity))
      ...
      <div>
        ...
        <div className="flex flex-wrap justify-center items-center">
          {users.map((user) => {
            ...
            return (
              <>
              <div key={user.sid} className="m-2">
                <ParticipantTile
                  user={member!}
                  participant={user}
                  audioTrack={
                    tracks.find((t) => t.user.identity === user.identity)?.track
                      ?.mediaStreamTrack
                  }
                />
              </div>
