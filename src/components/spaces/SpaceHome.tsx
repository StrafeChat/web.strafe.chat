      ...
      <div class="relative h-[120px] overflow-hidden">
        ...
        <div class="absolute bottom-5 items-center left-4 flex gap-4">
          <div class="w-16 h-16 rounded-full bg-white bg-opacity-20 backdrop-blur-sm flex items-center justify-center overflow-hidden">
            <Show when={props.space?.icon} fallback={
              <span class="text-2xl font-bold text-white">
                {props.space?.name_acronym || props.space?.name?.charAt(0) || "S"}
              </span>
            }>
              <img
      ...
      <div class="p-6">
        <div class="mb-6">
          ...
          <div class="flex gap-4 text-sm text-text-secondary mb-4">
            <span>Created: {props.space?.created_at ? new Date(props.space.created_at).toLocaleDateString() : 'Unknown'}</span>
            <span>•</span>
        ...
        <div class="bg-background1 rounded-lg p-6 mb-6">
          ...
          <p class="text-text-secondary mb-4">
            This is the home page for your space. Here you can see an overview of your community,
            recent activity, and important announcements.
          </p>
          
          {/* Quick actions */}
          <div class="flex flex-wrap gap-3">
            <button class="px-4 py-2 bg-primary text-white rounded-md hover:bg-primary-hover transition-colors text-sm font-medium">
              Browse Rooms
            </button>
            <button class="px-4 py-2 bg-surface bg-opacity-20 text-text-primary rounded-md hover:bg-opacity-30 transition-colors text-sm font-medium">
              View Members
            </button>
        ...
        <div class="bg-background1 rounded-lg p-6">
          ...
          <div class="text-text-secondary text-center py-8">
            <svg xmlns="http://www.w3.org/2000/svg" class="w-12 h-12 mx-auto mb-3 opacity-50" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
              <circle cx="12" cy="12" r="10" />
              <polyline points="12,6 12,12 16,14" />
            </svg>
            <p>No recent activity to show</p>
            <p class="text-sm mt-1">Activity will appear here as members interact in your space</p>
          </div>
        </div>
      </div>
    </div>
