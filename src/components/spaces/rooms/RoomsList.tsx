...
const DropZone: Component<DropZoneProps> = (props) => {
  const isActive = () => {
    ...
    if (active) {
      console.log(
        "[DropZone] Zone is active:",
        props.zoneId,
        "acceptType:",
        props.acceptType,
        "draggedType:",
        globalDragState().draggedType,
      );
    }
    return active;
  };
  const isDropTarget = () =>
    globalDragState().dragOverId === props.zoneId &&
    globalDragState().dragOverType === "zone";
...
const DraggableSection: Component<DraggableSectionProps> = (props) => {
  ...
  const isDropTarget = () =>
    globalDragState().dragOverId === props.section.id &&
    globalDragState().dragOverType === "section";
  const isRoomDropTarget = () =>
    isDropTarget() && globalDragState().draggedType === "room";
  const isSectionDropTarget = () =>
    isDropTarget() && globalDragState().draggedType === "section";
  ...
  const handleSectionDragOver = (e: DragEvent) => {
    ...
    if (
      (draggedType === "section" &&
        globalDragState().draggedId !== props.section.id) ||
      draggedType === "room"
    ) {
      setGlobalDragState((prev) => ({
        ...prev,
        dragOverId: props.section.id,
        dragOverType: "section",
      }));
    }
  };
  ...
  return (
    <div
      class={`transition-all duration-200 mt-3 ${
        ...
        isRoomDropTarget()
          ? "bg-blue-500 bg-opacity-20 border-2 border-blue-500 border-dashed rounded-md p-2"
          : isSectionDropTarget()
            ? "bg-green-500 bg-opacity-20 border-2 border-green-500 border-dashed rounded-md p-2"
            : ""
      }`}
    >
      <div
      ...
      >
          ...
          <DropZone
            ...
            onDrop={(draggedId) => {
              ...
              if (!draggedRoom) {
                ...
                props.onRoomMoveToSectionAtPosition?.(
                  draggedId,
                  props.section.id,
                  0,
                );
        ...
        <For each={sortedRooms()}>
          {(room, index) => {
            ...
            const isDraggingPrevRoom =
              prevRoom && draggedRoomId === prevRoom.id;
              ...
              <div>
                <Show when={!isDraggingThisRoom && !isDraggingPrevRoom}>
                  <DropZone
                    ...
                    onDrop={(draggedId) => {
                      const draggedRoom = props.rooms.find(
                        (r) => r.id === draggedId,
                      );
                      if (draggedRoom) {
                        ...
                        const targetIndex = props.rooms.findIndex(
                          (r) => r.id === room.id,
                        );
                        props.onRoomMoveToSectionAtPosition?.(
                          draggedId,
                ...
                <Show
                  when={
                    index() === sortedRooms().length - 1 && !isDraggingThisRoom
                  }
                >
                  <DropZone
                    acceptType="room"
...
const RoomsList: Component<RoomsListProps> = (props) => {
  ...
  const updateRoomParent = async (
    roomId: string,
    newParentId: string | null,
    newPosition: number,
  ) => {
    console.log("[RoomsList] updateRoomParent called with:", {
      roomId,
      newParentId,
      newPosition,
    });
    try {
      ...
      console.log(
        "[RoomsList] Sending request body:",
      ...
      if (!response.ok) {
        throw new Error(
          `Failed to update room parent: ${response.status} ${responseText}`,
        );
      }
  ...
  const spaceRooms = createMemo(() => {
    console.log(
      "[RoomsList] spaceRooms memo triggered, props.spaceId:",
      props.spaceId,
    );
    if (!props.spaceId) return [];
    const allRooms = rooms();
    console.log(
      "[RoomsList] spaceRooms allRooms from AuthProvider:",
      allRooms?.length || 0,
    );
    if (!allRooms) return [];
    ...
    console.log(
      "[RoomsList] spaceRooms filtered result:",
      filtered.length,
      "rooms:",
      filtered.map((r) => ({
        id: r.id,
  ...
  const sections = createMemo(() => {
    const sectionRooms = spaceRooms().filter(
      (room: RoomWithRecipients) => room.type === RoomType.SPACE_SECTION,
    );
    ...
    console.log(
      ...
      sorted.map((s) => ({ name: s.name, position: s.position, id: s.id })),
    );
  ...
  const allRoomsInSpace = createMemo(() => {
    console.log(
      "[RoomsList] allRoomsInSpace memo triggered, spaceRooms:",
      spaceRooms().length,
    );
    const filtered = spaceRooms().filter(
      (room: RoomWithRecipients) =>
        room.type === RoomType.TEXT_ROOM || room.type === RoomType.VOICE_ROOM,
    );
    console.log(
      "[RoomsList] allRoomsInSpace filtered:",
      filtered.length,
      "rooms with parent_ids:",
      filtered.map((r) => ({
        id: r.id,
        parent_id: r.parent_id,
        position: r.position,
  ...
  createEffect(() => {
    ...
    const signature = currentRooms
      .map((r) => `${r.id}:${r.parent_id}:${r.position}`)
      .join("|");
  ...
  const roomsBySection = createMemo(() => {
    ...
    console.log(
      "[RoomsList] roomsBySection memo triggered, allRoomsInSpace:",
      allRoomsInSpace().length,
    );
    ...
    console.log(
      "[RoomsList] roomsBySection grouped:",
  ...
  const orphanedRooms = createMemo(() => {
    ...
    const currentRooms = allRoomsInSpace();
    const orphaned = currentRooms
      .filter((room: RoomWithRecipients) => !room.parent_id)
  ...
  const handleRoomMoveToSection = async (
    draggedRoomId: string,
    targetSectionId: string,
  ) => {
    console.log("[RoomsList] Moving room to section:", {
      draggedRoomId,
      targetSectionId,
    });
  ...
  const handleRoomMoveToSectionAtPosition = async (
    draggedRoomId: string,
    targetSectionId: string,
    position: number,
  ) => {
    console.log("[RoomsList] Moving room to section at position:", {
      draggedRoomId,
      targetSectionId,
      position,
    });
  ...
  const handleRoomOrphan = async (
    ...
    if (targetRoomId && position) {
      ...
      const targetIndex = currentOrphaned.findIndex(
        (r) => r.id === targetRoomId,
      );
      ...
      <div class="flex flex-col gap-0.5 p-2 flex-1 overflow-y-auto min-h-0">
                ...
                <div>
                  <Show when={!isDraggingThisRoom && !isDraggingPrevRoom}>
                    <DropZone
                      ...
                      onDrop={(draggedId) =>
                        handleRoomReorder(draggedId, room.id, "before")
                      }
                    />
                  </Show>
                  <OrphanedRoomItem
                  ...
                  >
                    <DropZone
                      ...
                      onDrop={(draggedId) =>
                        handleRoomReorder(draggedId, room.id, "after")
                      }
                    />
                  </Show>
                </div>
        ...
        <For each={sections()}>
          {(section, index) => {
            ...
            const isDraggingPrevSection =
              prevSection && draggedSectionId === prevSection.id;
              ...
              <div>
                <Show when={!isDraggingThisSection && !isDraggingPrevSection}>
                  <DropZone
                    ...
                    onDrop={(draggedId) =>
                      handleSectionReorder(draggedId, section.id, "before")
                    }
                  />
                </Show>
                <DraggableSection
                ...
                <Show
                  when={
                    index() === sections().length - 1 && !isDraggingThisSection
                  }
                >
                  <DropZone
                    acceptType="section"
                    ...
                    onDrop={(draggedId) =>
                      handleSectionReorder(draggedId, section.id, "after")
                    }
                  />
                </Show>
              </div>
