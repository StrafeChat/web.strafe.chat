...
export class WebSocketClient {
  ...
  private async handleRoomCreate(payload: any): Promise<void> {
          ...
          for (let i = 0; i < missingUserIds.length; i += 100) {
            ...
            if (response.ok) {
              ...
              if (users && users.users) {
                Object.entries(users.users).forEach(([_, user]) => {
                  this.cache.setUser({
                    id: user.ID,
                    username: user.Username,
                    discriminator: user.Discriminator,
    ...
    // Dispatch the room create event
  ...
  private handleReady(data: ReadyPayload): void {
    ...
    // Set connected state and start heartbeat for direct connections
    this.connected = true;
    this.notifyConnectionState();
    if (!WebSocketClient.isSharedWorkerSupported) {
      this.startHeartbeat();
    ...
    this.connectPromise = new Promise((resolve) => {
      if (WebSocketClient.isSharedWorkerSupported && this.worker) {
        ...
        const connectionTimeout = setTimeout(() => {
          ...
          this.notifyConnectionState();
          resolve(false);
          // Reset connect promise so we can try again
        ...
        const connectionHandler = (connected: boolean) => {
          ...
          resolve(connected);
          // Reset connect promise only after we've resolved
          setTimeout(() => {
            this.connectPromise = null;
          }, 100);
        };
        ...
        const connectionTimeout = setTimeout(() => {
          ...
          // If we're in a timeout situation, try to reconnect
          if (this.currentToken) {
            this.scheduleReconnect();
          }
        }, 10000); // 10 second timeout
