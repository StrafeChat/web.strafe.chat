...
export const clearUpdateCache = () => {
  localStorage.removeItem('strafe_chat_shown_updates');
...
export const simulateUpdate = () => {
  ...
  window.dispatchEvent(new CustomEvent('test-update-notification', {
    detail: fakeUpdate
  }));
...
export const getUpdateCacheStatus = () => {
  try {
    const stored = localStorage.getItem('strafe_chat_shown_updates');
